...
function App() {
  ...
  // State to track the scroll button
  const [showScrollButton, setShowScrollButton] = useState(false);
	...
	const removeFilterPill = (type, value) => {
		if (type === "level") {
			...
			setSelectedInstructors((prev) =>
				prev.filter((instructor) => instructor !== value)
			);
		} else if (type === "search") {
			setSearchTerm("");
	...
	const filteredCourses = coursesData
		.filter((course) => {
			...
			const selectedMatch =
				!selectedOutlineCourse || course.id === selectedOutlineCourse;
	...
	const totalPages = showAllCourses
		? 1
  ...
  useEffect(() => {
    const handleScroll = () => {
      // Show the button if the page is scrolled down even a little bit
				...
				<div className="header-content">
					<div className="header-left">
						...
						<p className="subtitle">
							Explore undergraduate Computer Science courses
						</p>
				...
				<aside className="filters-sidebar">
					<div className="filters accordion-filters">
						...
						<div className="accordion-item filter-dropdown-container">
							...
							<div
								className={`accordion-content ${
									openDropdown === "semester" ? "expanded" : ""
								}`}
							>
								{["F", "W", "SP", "SU"].map((sem) => (
						...
						<div className="accordion-item filter-dropdown-container">
							...
							<div
								className={`accordion-content ${
									openDropdown === "instructor" ? "expanded" : ""
								}`}
							>
						...
						<div className="active-filters">
							{searchTerm && (
								<div className="filter-pill">
									...
									<button
										onClick={() => removeFilterPill("search", searchTerm)}
									>
										×
									</button>
								</div>
							...
							{courseLevels.map((level) => (
								<div className="filter-pill" key={`pill-level-${level}`}>
									...
									<button onClick={() => removeFilterPill("level", level)}>
										×
									</button>
								</div>
							))}
							...
							{creditHours.map((credit) => (
								<div className="filter-pill" key={`pill-credit-${credit}`}>
									...
									<button onClick={() => removeFilterPill("credit", credit)}>
										×
									</button>
								</div>
							))}
							...
							{semesters.map((sem) => (
								<div className="filter-pill" key={`pill-semester-${sem}`}>
									...
									<button onClick={() => removeFilterPill("semester", sem)}>
										×
									</button>
								</div>
							))}
							...
							{showFavorites && (
								<div className="filter-pill">
									<span>Favorites</span>
									<button onClick={() => removeFilterPill("favorites")}>
										×
									</button>
								</div>
							)}
							...
							{selectedOutlineCourse && (
								<div className="filter-pill selected-course-pill">
									<span>
										Selected:{" "}
										{
											coursesData.find((c) => c.id === selectedOutlineCourse)
												?.header
										}
									</span>
									<button onClick={() => removeFilterPill("selected")}>
										×
									</button>
								</div>
							)}
							...
							{selectedInstructors.map((instructor) => (
								<div
									...
									key={`pill-instructor-${instructor}`}
								>
									<span>{instructor}</span>
									<button
										onClick={() => removeFilterPill("instructor", instructor)}
									>
										×
									</button>
								</div>
								...
								displayedCourses.map((course) => (
									<div
										className={`course-card ${
											course.id === selectedOutlineCourse
												? "highlighted-course"
												: ""
										}`}
										key={course.id}
								...
								{coursesData
									...
									.map((course) => (
										<li
											key={course.id}
											className={`${
												favorites.includes(course.id) ? "favorited-course" : ""
											} ${
												course.id === selectedOutlineCourse
						...
						<div className="outline-section">
							...
							<div className="progress-container">
								<div className="progress-info">
									...
									<div className="progress-value">
										{
											coursesData.filter(
												(c) => c.isCore && favorites.includes(c.id)
											).length
										}{" "}
										/ {coursesData.filter((c) => c.isCore).length}
									</div>
								</div>
							...
							<div className="total-credits">
								...
								<div className="credits-container">
									<p>
										<strong>Favorited Credits:</strong>{" "}
										{coursesData
											.filter((c) => favorites.includes(c.id))
											.reduce((sum, course) => sum + course.credits, 0)}
									</p>
									<p>
										<strong>Total Required:</strong> 120
									</p>
								</div>
							</div>
						</div>
				...
				<div className="modal-overlay" onClick={closeModal}>
					<div className="modal-content" onClick={(e) => e.stopPropagation()}>
						...
						<div className="modal-inner-content">
							...
							>
								{favorites.includes(modalCourse.id)
									? "★ Unfavorite"
									: "☆ Favorite"}
							</button>
        ...
        >
          ...
          >
			...
			<footer className="footer">
				...
				<p>
					Contact: <a href="mailto:webmaster@byu.edu">webmaster@byu.edu</a> |
